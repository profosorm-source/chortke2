...
class UserLevelService
{
    ...
    public function checkUpgrade(int $userId): ?string
    {
        ...
        logger('info', 'User level upgraded by activity', [
            'user_id' => $userId,
    ...
    public function purchaseLevel(int $userId, string $levelSlug, string $currency = 'irt'): array
    {
        ...
        try {
            ...
            $txId = $this->walletService->withdraw(
                ...
                'vip_purchase',
                "خرید سطح {$level->name}",
                "level_purchase_{$userId}_{$levelSlug}_" . \time()
            );
            ...
            logger('info', 'User level purchased', [
                'user_id' => $userId,
                'level' => $levelSlug,
                'price' => $price,
                'currency' => $currency,
            ...
            $this->db->rollBack();
            logger('error', 'Level purchase failed', ['error' => $e->getMessage(), 'user_id' => $userId]);
    ...
    public function checkDowngrades(): array
    {
        ...
        foreach ($inactiveUsers as $user) {
            ...
            logger('info', 'User level downgraded for inactivity', [
                'user_id' => $user->id,
                'from' => $user->level_slug,
    ...
    public function monthlyReset(): int
    {
        ...
        logger('info', 'Monthly active days reset', ['affected' => $count]);
        return $count;
    }
