...
class LotteryService
{
    ...
    public function createRound(int $adminId, array $data): array
    {
        ...
        try {
            ...
            $this->db->rollBack();
            logger('lottery_round_error', $e->getMessage(), 'error');
    ...
    public function participate(int $userId, int $roundId): array
    {
        ...
        try {
            ...
            $this->db->rollBack();
            logger('lottery_participation_error', $e->getMessage(), 'error');
    ...
    public function generateDailyNumbers(int $roundId): array
    {
        ...
        try {
            ...
            logger('lottery_daily_numbers', "Round {$roundId}, date {$today}, type {$matchType}", 'info');
            ...
            $this->db->rollBack();
            logger('lottery_daily_error', $e->getMessage(), 'error');
            return ['success' => false, 'message' => 'خطای سیستمی.'];
        }
    }
    ...
    public function vote(int $userId, int $dailyNumberId, int $selectedNumber): array
    {
        ...
        try {
            ...
            $this->db->commit();
            logger('lottery_vote', "User {$userId}, daily {$dailyNumberId}, number {$selectedNumber}", 'info');

            return ['success' => true, 'message' => 'رأی شما ثبت شد!', 'vote_id' => $voteId];

        } catch (\Throwable $e) {
            $this->db->rollBack();
            logger('lottery_vote_error', $e->getMessage(), 'error');
            return ['success' => false, 'message' => 'خطای سیستمی.'];
        }
    }
    ...
    public function updateChanceScores(int $roundId, string $date): array
    {
        ...
        try {
            ...
            logger('lottery_chance_updated', "Round {$roundId}, updated {$updatedCount}", 'info');
            ...
            $this->db->rollBack();
            logger('lottery_chance_error', $e->getMessage(), 'error');
            return ['success' => false, 'message' => 'خطای سیستمی.'];
        }
    }
    ...
    public function selectWinner(int $roundId, int $adminId): array
    {
        ...
        try {
            ...
            logger('lottery_winner', "Round {$roundId}, winner {$winner->user_id}, score {$winner->chance_score}", 'info');
    ...
    public function cancelRound(int $roundId, int $adminId, string $reason = ''): array
    {
        ...
        try {
            ...
            logger('lottery_cancelled', "Round {$roundId} by admin {$adminId}", 'info');

            return ['success' => true, 'message' => 'دوره لغو و هزینه‌ها بازگشت داده شد.', 'refunded_count' => count($participants)];

        } catch (\Throwable $e) {
            $this->db->rollBack();
            logger('lottery_cancel_error', $e->getMessage(), 'error');
            return ['success' => false, 'message' => 'خطای سیستمی.'];
        }
    }
