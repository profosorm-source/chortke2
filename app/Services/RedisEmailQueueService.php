...
class RedisEmailQueueService
{
    ...
    public function push(array $emailData): bool|string
    {
        ...
        if ($this->useRedis) {
            try {
                ...
                logger('info', "Email queued in Redis: {$emailId}");
                return $emailId;
            } catch (\Throwable $e) {
                logger('error', 'Redis email queue failed: ' . $e->getMessage());
    ...
    public function pop(int $limit = 10): array
    {
        if ($this->useRedis) {
            try {
                ...
                logger('error', 'Redis email pop failed: ' . $e->getMessage());
    ...
    public function markAsSent(string $emailId): bool
    {
        if ($this->useRedis) {
            try {
                ...
                logger('info', "Email sent and archived: {$emailId}");
                return true;
            } catch (\Throwable $e) {
                logger('error', 'Redis mark as sent failed: ' . $e->getMessage());
                return false;
            }
        }
            ...
            try {
                ...
                if ($data) {
                    ...
                    if ($email['attempts'] >= 3) {
                        ...
                        logger('warning', "Email failed after 3 attempts: {$emailId}");
                    } else {
                        ...
                        logger('info', "Email retry scheduled: {$emailId} (attempt {$email['attempts']})");
                    }
                }

                return true;
            } catch (\Throwable $e) {
                logger('error', 'Redis mark as failed error: ' . $e->getMessage());
                return false;
            }
        }
    ...
    public function getStats(): array
    {
        if ($this->useRedis) {
            try {
                ...
                logger('error', 'Redis stats error: ' . $e->getMessage());
            }
        }
    ...
    public function cleanup(): int
    {
        if ($this->useRedis) {
            try {
                ...
                logger('info', "Redis email cleanup: {$cleaned} emails removed");
                return $cleaned;
            } catch (\Throwable $e) {
                logger('error', 'Redis cleanup error: ' . $e->getMessage());
                return 0;
            }
        }
    ...
    private function fallbackToDatabase(array $payload): bool|string
    {
        try {
            ...
            logger('error', 'Database email queue failed: ' . $e->getMessage());
            return false;
        }
    }
    ...
    private function fallbackGetFromDatabase(int $limit): array
    {
        try {
            ...
            logger('error', 'Database email get failed: ' . $e->getMessage());
            return [];
        }
    }
    ...
    private function fallbackMarkAsSentInDatabase(string $emailId): bool
    {
        try {
            ...
            logger('error', 'Database mark as sent failed: ' . $e->getMessage());
            return false;
        }
    }
    ...
    private function fallbackMarkAsFailedInDatabase(string $emailId, string $error): bool
    {
        try {
            ...
            logger('error', 'Database mark as failed error: ' . $e->getMessage());
            return false;
        }
    }
    ...
    private function fallbackGetStatsFromDatabase(): array
    {
        try {
            ...
            logger('error', 'Database stats error: ' . $e->getMessage());
    ...
    private function archiveToDatabase(array $email): void
    {
        try {
            ...
            logger('error', 'Email archive to DB failed: ' . $e->getMessage());
        }
    }
}
