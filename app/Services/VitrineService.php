...
class VitrineService
{
    ...
    public function createListing(int $userId, array $data): array
    {
        ...
        AuditTrail::record('vitrine.listing_created', $userId, [
    ...
    public function lockEscrow(int $buyerId, int $listingId): array
    {
        ...
        AuditTrail::record('vitrine.escrow_locked', $buyerId, [
            'listing_id' => $listingId,
            'amount'     => $finalPrice,
            'deadline'   => $deadline,
        ]);
        log_activity('vitrine.escrow_locked',
            "پرداخت escrow ویترین انجام شد — مبلغ: {$finalPrice} USDT",
            $buyerId,
            ['listing_id' => $listingId, 'amount' => $finalPrice]
        );
    ...
    public function releaseFundsToSeller(object $listing, string $reason = 'manual'): array
    {
        ...
        AuditTrail::record('vitrine.funds_released', (int) $listing->seller_id, [
            'listing_id' => $listing->id,
            'net'        => $net,
            'reason'     => $reason,
        ]);
        log_activity('vitrine.funds_released',
            "وجه ویترین آزاد شد — {$net} USDT به فروشنده پرداخت شد",
            (int) $listing->seller_id,
            ['listing_id' => $listing->id, 'net' => $net, 'reason' => $reason]
        );
    ...
    public function openDispute(int $userId, int $listingId, string $reason): array
    {
        ...
        log_activity('vitrine.dispute_opened',
    ...
    public function resolveDispute(int $listingId, string $winner, int $adminId): array
    {
        ...
        AuditTrail::record('vitrine.dispute_resolved', $adminId, [
            'listing_id' => $listingId,
            'winner'     => $winner,
        ]);
