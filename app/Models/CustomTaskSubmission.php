...
public function find(int $id): ?object
    ...
    public function create(array $d): ?object
    {
        ...
        ");
        $result = $stmt->execute([
            $d['task_id'], $d['worker_id'], $d['deadline_at'],
            $d['status'] ?? 'in_progress', $d['reward_amount'] ?? 0,
            $d['reward_currency'] ?? 'irt', $d['idempotency_key'],
    ...
    {
        $fields = []; $values = [];
        $allowed = ['proof_text','proof_file','proof_file_hash','submitted_at','reviewed_at',
    ...
    public function getByTask(int $taskId, ?string $status = null, int $limit = 30, int $offset = 0): array
    {
        ...
        $stmt = $this->db->prepare("
            ...
            FROM custom_task_submissions s
            LEFT JOIN users w ON w.id = s.worker_id
            WHERE {$whereStr} ORDER BY s.created_at DESC LIMIT ? OFFSET ?
        ");
    ...
    {
        return [
            'in_progress' => 'در حال انجام', 'submitted' => 'ارسال شده',
            'approved' => 'تأیید شده', 'rejected' => 'رد شده',
            'expired' => 'منقضی', 'disputed' => 'در اختلاف',
        ];
    }
    ...
    {
        return [
            'in_progress' => 'badge-info', 'submitted' => 'badge-warning',
            'approved' => 'badge-success', 'rejected' => 'badge-danger',
            'expired' => 'badge-secondary', 'disputed' => 'badge-danger',
        ];
    }
}