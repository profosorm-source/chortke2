...
class IdempotencyKey
{
    ...
    public function check(string $key, int $userId, string $action, ?array $requestData = null, int $retryCount = 0): array
    {
        ...
        try {
            ...
            if ($wasInserted) {
                error_log("[{$logId}] New idempotency key: {$key} | Action: {$action}");
                return ['is_duplicate' => false];
            }

            // ردیف قبلاً وجود داشت — بررسی وضعیت
            error_log("[{$logId}] Existing key found: {$key} | Status: {$existing['status']}");
            ...
            if ($e->getCode() == 23000 || str_contains($e->getMessage(), 'Duplicate entry')) {
                error_log("[{$logId}] Duplicate key race condition for: {$key}. Retry #{$retryCount}");
            ...
            error_log("[{$logId}] Database error in idempotency check: " . $e->getMessage());
    ...
    public function complete(string $key, $result, ?int $userId = null): bool
    {
        try {
            ...
            if ($success) {
                error_log("✅ Idempotency key marked as completed: {$key}");
            }
            
            return $success;
            
        } catch (\PDOException $e) {
            error_log("❌ Error completing idempotency key {$key}: " . $e->getMessage());
            return false;
        }
    }
    ...
    public function fail(string $key, $error, ?int $userId = null): bool
    {
        try {
            ...
            if ($success) {
                error_log("❌ Idempotency key marked as failed: {$key}");
            }
            
            return $success;
            
        } catch (\PDOException $e) {
            error_log("❌ Error marking idempotency key as failed {$key}: " . $e->getMessage());
            return false;
        }
    }
    ...
    public function cleanup(bool $dryRun = false): int
    {
        try {
            ...
            if ($deleted > 0) {
                error_log("🧹 Idempotency keys cleanup: {$deleted} keys removed (older than {$expiryDate})");
            }
            
            return $deleted;
            
        } catch (\PDOException $e) {
            error_log("❌ Error in idempotency cleanup: " . $e->getMessage());
            return 0;
        }
    }
    ...
    public static function wrap(string $key, int $userId, string $action, callable $callback, ?array $requestData = null)
    {
        ...
        error_log("[{$logId}] 🔐 Idempotency wrap started | Key: {$key} | User: {$userId} | Action: {$action}");
        
        // بررسی کلید
        $check = $service->check($key, $userId, $action, $requestData);
        
        if ($check['is_duplicate']) {
            error_log("[{$logId}] 🔁 Duplicate request detected, returning cached result");
            return $check['result'];
        }
        
        try {
            // اجرای عملیات
            error_log("[{$logId}] ▶️ Executing callback...");
            $result = $callback();
            
            // ذخیره نتیجه موفق
            $service->complete($key, $result, $userId);
            
            error_log("[{$logId}] ✅ Callback executed successfully");
            ...
            error_log("[{$logId}] ❌ Callback failed: " . $e->getMessage());
    ...
    public function getStats(): array
    {
        try {
            ...
            error_log("❌ Error getting idempotency stats: " . $e->getMessage());
