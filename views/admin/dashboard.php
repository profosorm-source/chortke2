            ...
            <div class="table-wrap">
                <table>
                    ...
                    <?php foreach ($pendingWithdrawalsList as $w):
                        $wg = $gradients[($w->user_id ?? 0) % count($gradients)];
                        $wi = mb_substr($w->full_name ?? 'ک', 0, 1, 'UTF-8');
                    ?>
                        <tr>
                            <td>
                                <div class="user-cell">
                            ...
                            <td><span class="amount-cell positive"><?= fa_number(number_format((int)($w->amount ?? 0))) ?></span></td>
                            <td style="font-size:12px;color:var(--text-muted)"><?= e($w->bank_name ?? '-') ?></td>
                            <td style="font-size:11px;color:var(--text-muted)"><?= jdate('Y/m/d H:i', strtotime($w->created_at ?? 'now')) ?></td>
                            <td>
            ...
            <div class="table-wrap">
                <table>
                    ...
                    <?php foreach ($recentUsers as $u):
                        $ug = $gradients[($u->id ?? 0) % count($gradients)];
                        $ui = mb_substr($u->full_name ?? 'ک', 0, 1, 'UTF-8');
                    ?>
                        <tr>
                            <td>
                                <div class="user-cell">
                            ...
                            <td style="font-size:12px;direction:ltr"><?= e($u->mobile ?? '-') ?></td>
                            <td><span class="badge <?= $roleColors[$u->role ?? ''] ?? 'badge-muted' ?>"><?= $roleNames[$u->role ?? ''] ?? 'کاربر' ?></span></td>
                            <td><span class="badge <?= $statusColors[$u->status ?? ''] ?? 'badge-muted' ?>"><?= $statusNames[$u->status ?? ''] ?? '-' ?></span></td>
                            <td style="font-size:11px;color:var(--text-muted)"><?= jdate('Y/m/d', strtotime($u->created_at ?? 'now')) ?></td>
                            <td>
                                <div class="action-btns">
                                    <a href="<?= url('/admin/users/'.$u->id) ?>"       class="icon-btn view"   title="مشاهده"><span class="material-icons">visibility</span></a>
                                    <a href="<?= url('/admin/users/edit/'.$u->id) ?>"  class="icon-btn edit"   title="ویرایش"><span class="material-icons">edit</span></a>
...
include __DIR__ . '/partials/sentry-widget.php';
?>
<?php
