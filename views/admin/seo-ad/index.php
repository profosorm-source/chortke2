  ...
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="page-title mb-1">
        <span class="material-icons text-primary">manage_search</span> مدیریت SEO Ad
      </h4>
      <p class="text-muted mb-0" style="font-size:12px;">
        تبلیغات SEO ثبت‌شده توسط کاربران — تایید یا رد کنید
      </p>
    </div>
    <a href="<?= url('/admin/seo-keywords') ?>" class="btn btn-outline-secondary btn-sm">
      <span class="material-icons" style="font-size:16px;vertical-align:middle;">key</span>
      کلمات کلیدی مدیر
    </a>
  </div>
        ...
        <table class="table table-hover mb-0">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                <div class="small"><?= number_format($item->budget ?? 0) ?> تومان</div>
              ...
              <td>
                <div class="small text-danger"><?= number_format($spent) ?> تومان</div>
                <div class="small text-muted"><?= number_format($item->clicks_count ?? 0) ?> کلیک</div>
              </td>
              <td>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <textarea id="rejectReason" class="form-control" rows="3"
          placeholder="دلیل رد را بنویسید..."></textarea>
      </div>
...
document.querySelectorAll('.btn-approve').forEach(btn => {
  btn.addEventListener('click', function() {
    if(!confirm('تایید این تبلیغ SEO Ad؟')) return;
...
document.querySelectorAll('.btn-pause-ad').forEach(btn => {
  btn.addEventListener('click', function() {
    if(!confirm('توقف این تبلیغ؟')) return;
